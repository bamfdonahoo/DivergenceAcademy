...
def line_conversion():
	with open ('10kprimes.txt','r') as t: # open source file to read prime numbers from
		...
		for p in rl: # for each line variable do:
			...
			hash_int=int(hashlib.sha256(p.encode()).hexdigest(),16) # encode variable as utf-8,hash the variable through hashlib.sha256, output 256bit hex digest, convert hex string to numerical int
